...
func (resource *recordURLResource) Create(context context.Context, request resource.CreateRequest, response *resource.CreateResponse) {
	...
	if err != nil {
		response.Diagnostics.AddAttributeError(path.Root("destination"), "Destination must be a valid URL", err.Error())
		return
	}

	domain := plan.Domain.ValueString()
	name := plan.Name.ValueString()
	record := api.URLRecord{Name: name, Destination: *url, Type: type_}
	...
	plan.Type = types.Int32Value(recordInfo.Type)
...
func (resource *recordURLResource) Read(context context.Context, request resource.ReadRequest, response *resource.ReadResponse) {
	...
	state.Type = types.Int32Value(recordInfo.Type)
...
func (resource *recordURLResource) Update(context context.Context, request resource.UpdateRequest, response *resource.UpdateResponse) {
	...
	record := api.URLRecord{Name: plan.Name.ValueString(), Destination: *url, Type: type_}
...
func validateType(diagnostics *diag.Diagnostics, type_ int32) bool {
	if type_ == 301 || type_ == 302 {
		return true
	}
	diagnostics.AddError(fmt.Sprintf("Invalid URL record type: %d", type_), "valid URL record types are 301 and 302 (HTTP moved status codes)")
	return false
}
